use std::{
    collections::HashMap, ffi::CString, fs::File, io::BufReader, slice, sync::{Arc, Mutex, MutexGuard}
};
use std::path::Path;
use std::io::Read;
...
use crate::resource_manager::ztfile::{ZTFileType, ZTFile, ztfile_to_raw_resource};
...
impl LazyResourceMap {
    ...
    fn drop_inner(resource: LazyResource) {
        let data = match resource.backing {
            ResourceBacking::LoadedZipFile{data, archive: _ } => {
                data
            },
    ...
    fn insert_lazy(file_name: String, archive: Arc<Mutex<ZtdArchive>>) {
        ...
        let mut binding = LAZY_RESOURCE_MAP.lock().unwrap();
        if let Some(existing) = binding.insert(file_name.clone().to_ascii_lowercase(), LazyResource {
    ...
    fn insert_custom(file_name: String, file_type: ZTFileType, data: u32) {
        let mut binding = LAZY_RESOURCE_MAP.lock().unwrap();
        if let Some(existing) = binding.insert(file_name.to_ascii_lowercase(), LazyResource {
    ...
    fn get(key: &str) -> anyhow::Result<Option<ConcreteResource>> {
        ...
        let (archive_name , data) = match resource.backing.clone() {
            ResourceBacking::LazyZipFile{archive} => {
                ...
                let ztfile = ZTFile::builder().file_name(resource.filename.clone()).file_size(file_buffer.len() as u32).type_(resource.type_).raw_data(file_buffer).build();
    ...
    fn loaded_len() -> usize {
        ...
        binding.values().filter(|x| matches!(x.backing, ResourceBacking::LoadedZipFile{..} | ResourceBacking::Custom{..})).count()
    }
